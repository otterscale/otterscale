...
juju_clouds() {
    ...
    if su "$username" -c 'juju clouds 2>/dev/null | grep -q "^maas-cloud[[:space:]]"'; then
        log "WARN" "Juju cloud already exists, skip juju cloud configuration..."
        #log "WARN" "Juju cloud already exists, updating juju cloud configuration..."
...
juju_credentials() {
    ...
    if su "$username" -c 'juju credentials 2>/dev/null | grep -q "^maas-cloud[[:space:]]"'; then
        log "WARN" "Juju credential already exists, skip juju credential configuration..."
        #log "WARN" "Juju credential already exists, updating juju credential configuration..."
...
bootstrap_juju() {
    if execute_non_user_cmd "$username" "juju show-controller maas-cloud-controller | grep -q . >/dev/null 2>&1"; then
        log "INFO" "Juju had already bootstarp, skipping..."
    else
...
create_scope() {
    ...
    if execute_non_user_cmd "$username" "juju show-model default" "Show juju model"; then
        log "INFO" "Juju default model already exist, skipping..."
    else
        execute_non_user_cmd "$username" "juju add-model default" "create default model"
    fi
}
...
# JuJu K8S
juju_add_k8s() {
    if execute_non_user_cmd "$username" "juju show-cloud cos-k8s | grep -q . >/dev/null 2>&1" "Show juju cloud"; then
        log "INFO" "Juju cloud cos-k8s already exist, skipping add-k8s to maas-cloud-controller."
    else
        ...
        if ! execute_non_user_cmd "$username" "juju add-k8s cos-k8s --controller maas-cloud-controller --client --debug" "execute juju add-k8s"; then
            error_exit "Failed execute juju add-k8s cos-k8s --controller maas-cloud-controller --client --debug"
        fi
    fi

    if execute_non_user_cmd "$username" "juju show-model cos" "Show juju model"; then
        log "INFO" "Juju cos model already exist, skipping..."
    else
        log "INFO" "Juju add-model cos"
        if ! execute_non_user_cmd "$username" "juju add-model cos cos-k8s --debug" "execute juju add-model"; then
            error_exit "Failed execute juju add-model cos cos-k8s --debug"
        fi

        log "INFO" "Juju deploy cos-lite"
        if ! execute_non_user_cmd "$username" "juju deploy cos-lite --trust --debug" "juju deploy cos-lite"; then
            error_exit "Failed execute juju deploy cos-lite --trust --debug"
        fi
    fi
}
