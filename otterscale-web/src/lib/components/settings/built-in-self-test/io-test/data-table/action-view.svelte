		...
		<Form.Root>
			{#if testResult?.kind?.value?.output && testResult.kind.case == 'fio'}
				...
				<Form.Fieldset>
					...
					{@const readMinLatency = formatLatencyNano(
						Number(readOutput?.latency?.minNanoseconds || 0)
					)}
					{@const readMaxLatency = formatLatencyNano(
						Number(readOutput?.latency?.maxNanoseconds || 0)
					)}
					...
					<Form.Description
						>Bandwidth Bytes: {readBandwidthBytes.value} {readBandwidthBytes.unit}</Form.Description
					>
					...
					<Form.Description
						>Min Latency: {readMinLatency.value} {readMinLatency.unit}</Form.Description
					>
					<Form.Description
						>Max Latency: {readMaxLatency.value} {readMaxLatency.unit}</Form.Description
					>
					<Form.Description
						>Mean Latency: {readMeanLatency.value} {readMeanLatency.unit}</Form.Description
					>
				</Form.Fieldset>
				<Form.Fieldset>
					...
					{@const writeMinLatency = formatLatencyNano(
						Number(writeOutput?.latency?.minNanoseconds || 0)
					)}
					{@const writeMaxLatency = formatLatencyNano(
						Number(writeOutput?.latency?.maxNanoseconds || 0)
					)}
					...
					<Form.Description
						>Bandwidth Bytes: {writeBandwidthBytes.value}
					...
					<Form.Description
						>Min Latency: {writeMinLatency.value} {writeMinLatency.unit}</Form.Description
					>
					<Form.Description
						>Max Latency: {writeMaxLatency.value} {writeMaxLatency.unit}</Form.Description
					>
					<Form.Description
						>Mean Latency: {writeMeanLatency.value} {writeMeanLatency.unit}</Form.Description
					>
				</Form.Fieldset>
				<Form.Fieldset>
					...
					{@const trimMinLatency = formatLatencyNano(
						Number(trimOutput?.latency?.minNanoseconds || 0)
					)}
					{@const trimMaxLatency = formatLatencyNano(
						Number(trimOutput?.latency?.maxNanoseconds || 0)
					)}
					...
					<Form.Description
						>Bandwidth Bytes: {trimBandwidthBytes.value} {trimBandwidthBytes.unit}</Form.Description
					>
					...
					<Form.Description
						>Min Latency: {trimMinLatency.value} {trimMinLatency.unit}</Form.Description
					>
					<Form.Description
						>Max Latency: {trimMaxLatency.value} {trimMaxLatency.unit}</Form.Description
					>
					<Form.Description
						>Mean Latency: {trimMeanLatency.value} {trimMeanLatency.unit}</Form.Description
					>
				</Form.Fieldset>
			{:else}
