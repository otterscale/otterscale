...
<script lang="ts" generics="TData, TValue">
	let {
		...
		selectedFacility,
		buckets
	}: { selectedScopeUuid: string; selectedFacility: string; buckets: Writable<Bucket[]> } =
		$props();
...
<Layout.Root>
	...
	<Layout.Controller>
		...
		<Layout.ControllerAction>
			<Create {selectedScopeUuid} {selectedFacility} bind:buckets />
		</Layout.ControllerAction>
	</Layout.Controller>
	<Layout.Viewer>
		<Table.Root>
			...
			<Table.Body>
				{#each table.getRowModel().rows as row (row.id)}
					<Table.Row data-state={row.getIsSelected() && 'selected'}>
						...
						{/each}
						<Table.Cell>
							<Actions {selectedScopeUuid} {selectedFacility} bucket={row.original} bind:buckets />
						</Table.Cell>
					</Table.Row>
				{:else}
					<Table.Row>
