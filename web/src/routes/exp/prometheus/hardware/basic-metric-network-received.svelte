<script lang="ts">
	...
	const query = $derived(
		`
		rate(node_network_receive_bytes_total{instance="juju-1eb21e-0-lxd-1", device!="lo"}[5m])
		`
	);
	...
	const colors = ['hsl(var(--color-primary))', 'hsl(var(--color-secondary))', 'hsl(var(--color-info))', 'hsl(var(--color-success))', 'hsl(var(--color-danger))'];
...
{#if mounted}
	...
	<Template.Area title="Network Received">
		{#snippet hint()}
			<p>Historical view of network received over the past hour</p>
		{/snippet}
		{#snippet content()}
			{#if data.length === 0}
				<NoData type="area" />
