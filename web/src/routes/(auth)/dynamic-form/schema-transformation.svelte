<script lang="ts">
	...
	import { isSelect, retrieveSchema, type SchemaDefinition, schemaHash } from '@sjsf/form/core';
	import { HashedKeyCache, LRUCache } from '@sjsf/form/lib/cache';
	import { isSchemaObject, transformSchemaDefinition } from '@sjsf/form/lib/json-schema';
	...
	const schema = transformSchemaDefinition<SchemaDefinition>(originalSchema, (copy) => {
		...
		const { [altKeyword]: _, properties = {}, dependencies = {}, required = [], ...rest } = copy;
		...
		dependencies[VIRTUAL_PROPERTY] = {
			oneOf: alt.map((def, i) => {
				...
				return typeof def === 'boolean' ? option : merger.mergeSchemas(def, option);
			})
		};
		return {
			...rest,
...
<SimpleForm
	...
	validator={{
		...
		validateFormValue(rootSchema, formValue) {
			const retrievedSchema = retrieveSchema(validator, merger, rootSchema, rootSchema, formValue);
	...
	extraUiOptions={fromFactories({
		hideTitle: ({ path }: Config) => path.at(-1) === VIRTUAL_PROPERTY || undefined
	})}
/>
