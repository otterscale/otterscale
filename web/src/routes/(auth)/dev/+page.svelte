<script lang="ts">
	import { type JsonObject, type JsonValue, toJson } from '@bufbuild/protobuf';
	...
	let data = $derived<Record<string, JsonValue>[]>(objects);

	const columns: ColumnDef<Record<string, JsonValue>>[] = $derived.by(() => [
		{
			id: 'select',
			header: ({ table }) =>
		...
		...(objects.length > 0 && objects[0]
			? Object.keys(objects[0]).map((key) => ({
					...
					cell: ({ row }: { row: Row<Record<string, JsonValue>> }) =>
						renderComponent(ExtendedTableCell, {
	...
	function handleDeleteRows() {
		...
		data = data.filter(
			(item) => !selectedRows.some((row) => row.original && item && row.original.id === item.id)
		);
