<script lang="ts">
	...
	import * as Chart from '$lib/components/ui/chart/index.js'; // Constants
	...
	interface Props {
		...
		data: Array<DataPoint> /** Optional chart configuration object */;
		chartConfig?: ChartConfig /** Time range for x-axis formatting (1h, 1d, 7d, 30d) */;
		timeRange?: TimeRange /** Additional CSS classes for the chart container */;
		class?: string /** Optional value formatter function that returns formatted value and unit */;
		valueFormatter?: (value: number) => { value: number; unit: string };
	}

	let { data, chartConfig, timeRange, valueFormatter, class: className = '' }: Props = $props(); // Derived reactive values

	const computedChartConfig = $derived(() => chartConfig ?? generateChartConfig(data));
	const series = $derived(() => getSeries(computedChartConfig())); // Check if data contains NaN values

	const hasNaNValues = $derived(() => {
...
>
	{#if hasNaNValues()}
		<div class="flex h-full w-full items-center justify-center">
			<span class="text-4xl font-bold text-foreground">NaN</span>
		</div>
	{:else}
		<AreaChart
			legend
