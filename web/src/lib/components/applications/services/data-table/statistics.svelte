<script lang="ts">
	...
	const filteredServices = $derived(table.getFilteredRowModel().rows.map((row) => row.original));
</script>
...
<div class="grid w-full gap-3 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-5">
	{#snippet Services()}
		...
		{@const services = filteredServices.length}
	...
	{#snippet Types()}
		...
		{@const types = new Set([...filteredServices.map((service) => service.type)])}
		<Card.Root class="relative overflow-hidden">
			<Card.Header class="gap-3">
				<Card.Title class="flex items-center gap-2 font-medium">
					<div
