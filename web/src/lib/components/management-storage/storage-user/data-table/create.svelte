<script lang="ts" module>
	...
	export type Request = {
		...
		unlimitedUserSize: boolean;
		maximumUserSize: number;
		unlimitedUserObjects: boolean;
		maximumUserObjects: number;

		enableBucketQuota: boolean;
		unlimitedBucketSize: boolean;
		maximumBucketSize: number;
		unlimitedBucketObjects: boolean;
	...
	export const maximumBucketsOptions: Writable<SingleSelect.OptionType[]> = writable([
		{
			value: 'disabled',
			label: 'Disabled',
			icon: 'ph:grid-four'
	...
	export const s3keys: Writable<SingleSelect.OptionType[]> = $state(
		writable([
			{
				value: '5P4LaQdvk4sIymb4z3iahf8QSCJRPODJo466PKHN',
				label: 'QY8RYKUKLHTX32FQLKNC',
				icon: 'ph:lock'
			},
			{
				value: 'z',
	...
	export const capacities: LayeredMultipleSelect.OptionType[] = [
		{
			value: 'users',
			label: 'users',
			icon: 'ph:cube',
			subOptions: [
				{ value: 'read', label: 'read', icon: 'ph:user' },
				{ value: 'write', label: 'write', icon: 'ph:user' },
				{ value: '*', label: '*', icon: 'ph:user' }
			]
		},
		{
			value: 'buckets',
