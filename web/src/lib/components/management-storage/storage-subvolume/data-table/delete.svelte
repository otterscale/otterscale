<script lang="ts" module>
	import type { DeleteSubvolumeRequest, Subvolume } from '$gen/api/storage/v1/storage_pb';
...
<AlertDialog.Root bind:open={stateController.state}>
	...
	<AlertDialog.Content>
		<AlertDialog.Header>Delete Subvolume</AlertDialog.Header>
		<Form.Root>
			<Form.Fieldset>
				<Form.Field>
				...
				<AlertDialog.Action
					onclick={() => {
						...
						storageClient
							...
							.then((r) => {
								toast.success(`Delete ${request.subvolumeName}`);
								storageClient
									.listSubvolumes({
										scopeUuid: selectedScope,
										facilityName: selectedFacility,
							...
							.catch((e) => {
								toast.error(`Fail to delete subvolume: ${e.toString()}`);
							})
							.finally(() => {
								reset();
							});
