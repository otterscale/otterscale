...
<script lang="ts">
	...
	const readQuery = $derived(
		`
		rate(ceph_osd_op_r{ceph_daemon=~"${selectedObjectStorageDaemon}",juju_model_uuid=~"${selectedScope}"}[4m])
		`
	);

	const writeQuery = $derived(
		`
		rate(ceph_osd_op_w{ceph_daemon=~"${selectedObjectStorageDaemon}",juju_model_uuid=~"${selectedScope}"}[4m])
		`
	);
