<script lang="ts" module>
	...
	export const subvolumeGroups: Writable<SingleSelect.OptionType[]> = writable([
		{
			value: 'subvolume_group_1',
			label: 'Subvolume Group 1',
			icon: 'ph:cube'
		}
	]);
</script>
...
<script lang="ts">
	let { data = $bindable() }: { data: Writable<Subvolume[]> } = $props();
...
<AlertDialog.Root bind:open={stateController.state}>
	...
	<AlertDialog.Content>
		...
		<Form.Root>
			...
			<Form.Fieldset>
				<Form.Field>
					<Form.Label>Subvolume Group</Form.Label>
					<SingleSelect.Root options={subvolumeGroups} bind:value={request.subvolumeGroup}>
						...
						<SingleSelect.Content>
							<SingleSelect.Options>
								...
								<SingleSelect.List>
									...
									<SingleSelect.Group>
										{#each $subvolumeGroups as option}
