		...
		<div class="flex items-center">
			<div class="flex w-full flex-col gap-0">
				<Item.Root class="w-full">
					{@const command = `docker tag ${sourceImage || 'SOURCE_IMAGE'}${tag ? `:${tag}` : '[:TAG]'} ${registryURL}/${repository || 'REPOSITORY'}${tag ? `:${tag}` : '[:TAG]'}`}
				...
				<Item.Root class="w-full">
					{@const command = `docker push ${registryURL}/${repository || 'REPOSITORY'}${tag ? `:${tag}` : '[:TAG]'}`}
					<Item.Media variant="icon">
						<Icon icon="ph:cloud" />
					</Item.Media>
					<Item.Content class="flex flex-col items-start">
...
{#if isLoaded}
	<div class="gap-2' flex flex-col"></div>
{/if}
