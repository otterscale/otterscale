...
let {
	...
	onMount(async () => {
		try {
			...
			const bufferResponse = await fetch(bufferQuery);
			serieses.set('buffer', bufferResponse);

			const cacheResponse = await fetch(cacheQuery);
			serieses.set('cache', cacheResponse);
				...
				<div class="h-[200px] w-full resize overflow-visible">
					<AreaChart
						...
						series={[
							...
							{ key: 'buffer', color: 'hsl(var(--color-info))' },
							{ key: 'cache', color: 'hsl(var(--color-success))' },
							{ key: 'free', color: 'hsl(var(--color-danger))' }
						]}
						legend={{
