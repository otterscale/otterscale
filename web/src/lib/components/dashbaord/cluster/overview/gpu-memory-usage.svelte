<script lang="ts">
	...
	let gpuMemoryUsages = $state([] as SampleValue[]);

	const gpuMemoryUsageConfigurations = {
		usage: { label: 'Usage', color: 'var(--chart-1)' }
	} satisfies Chart.ChartConfig;

	async function fetchGPUMemoryUsage() {
		...
		gpuMemoryUsages = response.result[0]?.values ?? [];
	}

	async function fetch() {
		try {
			await Promise.all([fetchGPUMemoryUsage()]);
		} catch (error) {
...
{#if !isLoaded}
	<Card.Root>
		<Card.Header class="h-[42px]">
			<Card.Title>GPU Memory Usage</Card.Title>
...
{:else}
	<Card.Root class="h-full gap-2">
		...
		<Card.Content>
			<Chart.Container class="h-[230px] w-full px-2 pt-2" config={gpuMemoryUsageConfigurations}>
				<AreaChart
					data={gpuMemoryUsages}
					x="time"
					xScale={scaleUtc()}
					yPadding={[0, 25]}
					series={[
						{
							key: 'usage',
							label: gpuMemoryUsageConfigurations.usage.label,
							color: gpuMemoryUsageConfigurations.usage.color
						}
					]}
					props={{
						area: {
