...
<script lang="ts">
	...
	const alert: Alert.AlertType = $derived({
		...
		action: () => {
			installPlugins(['model']);
		},
		variant: 'destructive',
	});
</script>

{#if $modelPlugins.filter((modelPlugin) => modelPlugin.current).length < $modelPlugins.length}
	<Alert.Root {alert}>
		<Alert.Icon />
		...
		<Alert.Description>
			<div class="space-y-1">
				...
				<div class="flex w-full flex-wrap gap-2">
					{#each $modelPlugins.filter((plugin) => !plugin.current) as plugin}
