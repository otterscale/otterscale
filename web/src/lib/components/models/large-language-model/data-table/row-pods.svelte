...
<script lang="ts">
	let { table, row }: { table: TableType<LargeLanguageModel>; row: Row<LargeLanguageModel> } = $props();
	...
	const ready = $derived(pods.filter((p) => (typeof p.ready === 'number' ? p.ready > 0 : Boolean(p.ready))).length);
				...
				<div class="grid w-fit grid-cols-3 gap-4">
					<div class="col-span-3 grid grid-cols-3 gap-12">
						...
						<div class="col-span-2 flex items-center gap-1">
							...
							<div>
								<h6 class="text-muted-foreground text-xs">{m.phase()}</h6>
					...
					<div class="col-span-3 grid grid-cols-3 gap-12">
						<div class="flex items-center gap-1">
							<Icon icon="ph:check-circle" class="text-chart-2 size-6" />
							<div>
								<h6 class="text-muted-foreground text-xs">{m.running()}</h6>
								<p class="text-base">{running}</p>
							</div>
						</div>
						<div class="flex items-center gap-1">
							<Icon icon="ph:stop-circle" class="text-chart-4 size-6" />
							<div>
								<h6 class="text-muted-foreground text-xs">{m.ready()}</h6>
								<p class="text-base">{ready}</p>
							</div>
						</div>
						<div class="flex items-center gap-1">
							<Icon icon="ph:warning-circle" class="text-chart-1 size-6" />
							<div>
								<h6 class="text-muted-foreground text-xs">{m.restarts()}</h6>
