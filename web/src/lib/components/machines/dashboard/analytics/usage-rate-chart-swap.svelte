<script lang="ts">
	...
	const queries = $derived({
		overall: `sum(node_memory_SwapTotal_bytes{instance=~"${fqdn}"})`,
		using: `sum(node_memory_SwapTotal_bytes{instance=~"${fqdn}"})-sum(node_memory_SwapFree_bytes{instance=~"${fqdn}"})`,
		total: `sum(node_memory_SwapFree_bytes{instance=~"${fqdn}"})`
	});
...
<Statistics.Root type="ratio">
	<Statistics.Header>
		<div class="flex justify-between gap-4">
			<Statistics.Title>{m.swap()}</Statistics.Title>
	...
	<Statistics.Background icon="ph:hard-drive" class="-top-20 right-20 size-10" />
</Statistics.Root>
