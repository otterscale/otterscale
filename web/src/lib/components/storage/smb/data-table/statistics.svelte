<script lang="ts">
	...
	let { table }: { table: Table<SMBShare> } = $props();

	const filteredSMBShares = $derived(table.getFilteredRowModel().rows.map((row) => row.original));
</script>

<div class="grid w-full gap-3 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
	{#snippet SMBShares()}
		{@const title = 'SMB Share'}
		{@const titleIcon = 'ph:chart-bar-bold'}
		{@const backgroundIcon = 'ph:cube'}
		{@const shares = filteredSMBShares.length}
		<Card.Root class="relative overflow-hidden">
			...
			<Card.Content class="lg:max-[1100px]:flex-col lg:max-[1100px]:items-start">
				<p class="text-7xl font-semibold">{shares}</p>
			</Card.Content>
			<div
