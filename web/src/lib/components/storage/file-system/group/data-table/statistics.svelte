<script lang="ts">
	...
	let { table }: { table: Table<SubvolumeGroup> } = $props();

	const filteredGroups = $derived(table.getFilteredRowModel().rows.map((row) => row.original));
</script>
	...
	{#snippet Groups()}
		...
		<Card.Root class="relative overflow-hidden">
			...
			<Card.Content class="lg:max-[1100px]:flex-col lg:max-[1100px]:items-start">
				<p class="text-7xl font-semibold">{groups}</p>
			</Card.Content>
			<div
