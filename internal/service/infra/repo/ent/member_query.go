...
func (mq *MemberQuery) QueryOrganization() *OrganizationQuery {
...
func (mq *MemberQuery) WithOrganization(opts ...func(*OrganizationQuery)) *MemberQuery {
	query := (&OrganizationClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Member, error) {
	...
	if mq.withUser != nil || mq.withOrganization != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withOrganization; query != nil {
		if err := mq.loadOrganization(ctx, query, nodes, nil,
			func(n *Member, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MemberQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Member, init func(*Member), assign func(*Member, *Organization)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Member)
	for i := range nodes {
