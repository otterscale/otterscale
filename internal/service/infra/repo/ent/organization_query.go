...
func (oq *OrganizationQuery) QueryTeams() *TeamQuery {
...
func (oq *OrganizationQuery) QueryInvitations() *InvitationQuery {
	query := (&InvitationClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) WithTeams(opts ...func(*TeamQuery)) *OrganizationQuery {
	query := (&TeamClient{config: oq.config}).Query()
...
func (oq *OrganizationQuery) WithInvitations(opts ...func(*InvitationQuery)) *OrganizationQuery {
	query := (&InvitationClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withMembers; query != nil {
		if err := oq.loadMembers(ctx, query, nodes,
			func(n *Organization) { n.Edges.Members = []*Member{} },
			func(n *Organization, e *Member) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
...
func (oq *OrganizationQuery) loadTeams(ctx context.Context, query *TeamQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Team)) error {
...
func (oq *OrganizationQuery) loadMembers(ctx context.Context, query *MemberQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Member)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (oq *OrganizationQuery) loadInvitations(ctx context.Context, query *InvitationQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Invitation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
