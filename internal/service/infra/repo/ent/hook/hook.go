...
// The JWKSFunc type is an adapter to allow the use of ordinary
// function as JWKS mutator.
type JWKSFunc func(context.Context, *ent.JWKSMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f JWKSFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.JWKSMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.JWKSMutation", m)
}
