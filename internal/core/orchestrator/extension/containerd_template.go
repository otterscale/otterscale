...
const (
	ContainerdConfigTemplate = `# Patched by OtterScale at %s
...
`
	...
	ContainerdConfigWithGPU = `[plugins."io.containerd.grpc.v1.cri".containerd.runtimes.nvidia]
          base_runtime_spec = ""
          cni_conf_dir = ""
          cni_max_conf_num = 0
          container_annotations = []
...
func renderContainerdTemplate(gpu bool) string {
	now := time.Now().Format(time.RFC3339)
	if gpu {
		return fmt.Sprintf(ContainerdConfigTemplate, now, "nvidia", ContainerdConfigWithGPU)
	}
	return fmt.Sprintf(ContainerdConfigTemplate, now, "runc", "")
}
