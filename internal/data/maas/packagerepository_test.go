...
func TestNewPackageRepository(t *testing.T) {
	conf := &config.Config{}
	maas := New(conf)
	repo := NewPackageRepository(maas)
	if repo == nil {
		t.Fatal("Expected PackageRepository repository to be created, but got nil")
	}
	// Verify it implements the interface
	var _ core.PackageRepositoryRepo = repo
}

func TestPackageRepository_InterfaceCompliance(t *testing.T) {
	// Test that packageRepository implements core.PackageRepositoryRepo interface
	conf := &config.Config{}
	maas := New(conf)
	repo := NewPackageRepository(maas)
	var _ core.PackageRepositoryRepo = repo
...
func TestPackageRepository_Structure(t *testing.T) {
	// Test the structure and method signatures
	conf := &config.Config{}
	maas := New(conf)
	repo := NewPackageRepository(maas)
	// Verify the repo is of the correct type
	pr, ok := repo.(*packageRepository)
	if !ok {
		t.Fatal("Expected *packageRepository, but got a different type")
	}
	if pr.maas == nil {
...
func TestPackageRepository_WithConfig(t *testing.T) {
	tests := []struct {
		name string
		conf *config.Config
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			params := &entity.PackageRepositoryParams{Name: "test-repo", URL: "http://example.com/repo"}
...
func TestPackageRepository_ErrorHandling(t *testing.T) {
	tests := []struct {
		name   string
...
func TestPackageRepository_MethodBehavior(t *testing.T) {
...
func TestPackageRepository_IntegrationPatterns(t *testing.T) {
	...
	t.Run("sequential_calls", func(t *testing.T) {
		// First list package repositories
		repos, err := repo.List(ctx)
		if err != nil {
			t.Logf("List error (expected): %v", err)
		} else {
			t.Logf("List returned %d package repositories", len(repos))
		}
		// Then update a package repository
		params := &entity.PackageRepositoryParams{Name: "test-repo", URL: "http://example.com/repo"}
		_, err = repo.Update(ctx, 1, params)
		if err != nil {
...
func BenchmarkPackageRepository_Creation(b *testing.B) {
...
func BenchmarkPackageRepository_MethodCalls(b *testing.B) {
...
func TestPackageRepository_ConcurrentAccess(t *testing.T) {
...
func TestPackageRepository_TypeAssertions(t *testing.T) {
	...
	pr, ok := repo.(*packageRepository)
	if !ok {
		t.Fatal("Could not cast to *packageRepository")
	}
	if pr.maas != maas {
		t.Error("packageRepository.maas field not set correctly")
	}
	// Should implement the interface
	var _ core.PackageRepositoryRepo = pr
}

func TestPackageRepository_EdgeCases(t *testing.T) {
	t.Run("nil_maas", func(t *testing.T) {
		defer func() {
			if r := recover(); r == nil {
				t.Error("Expected panic when creating packageRepository with nil MAAS")
			}
		}()
		// This should panic or handle gracefully
		repo := &packageRepository{maas: nil}
		ctx := context.Background()
		_, _ = repo.List(ctx)
	})
